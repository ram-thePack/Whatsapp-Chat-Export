...
const safeWords = ['fuck','asshole','murder','killer','fundraising','funding','funds','mating','breeding'];
...
client.on("message", async (msg) => {
  ...
  if (msg.body.toLowerCase().includes("@askkai")) {
    ...
    (async () => {
      ...
      const { isNutrition, ingredients } = await analyzeMessage(message);
      ...
      if (isNutrition && ingredients) {
        ...
        if (repMsg && repMsg !== "") {
          msg.reply(
            `${repMsg}\n\nFor more information please send a message to KAi on +917760400141`,
          );
        }
      }
    })();
